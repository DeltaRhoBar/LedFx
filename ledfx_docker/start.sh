...
if [ -z "$PULSECLIENTMODE" ]; then
  ...
  pulseaudio --daemonize=yes --disallow-module-loading --log-target=stderr --disable-shm=yes --exit-idle-time=-1 --load="module-native-protocol-tcp" --load="module-always-sink"
else
