...
if [ -z "$PULSECLIENTMODE" ]; then
  ...
  pulseaudio --daemonize=yes --disallow-module-loading --log-target=stderr --disable-shm=yes --exit-idle-time=-1 --load="module-native-protocol-unix auth-anonymous=1 auth-cookie-enabled=0 socket=tcp:host.containers.internal:4713" --load="module-always-sink"
else
